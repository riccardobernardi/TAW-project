System.register(['http', 'mongoose', 'express', 'body-parser', 'passport', 'passport-http', 'jsonwebtoken', 'express-jwt', 'cors', './Table', './User'], function(exports_1) {
    var http, mongoose, express, bodyparser, passport, passportHTTP, jsonwebtoken, jwt, cors, table, user;
    var result, ios, app, auth;
    return {
        setters:[
            function (_http) {
                http = _http;
            ...
            function (_passportHTTP) {
                passportHTTP = _passportHTTP;
            },
        ...
        execute: function() {
            result = require('dotenv').config({ path: __dirname + '/.env' }); // The dotenv module will load a file named ".env"
            ios = undefined;
            app = express();
            ...
            app.route("/tables/:id", auth).get(function (req, res, next) {
                ...
                if (!sender.hasDeskRole() && !sender.hasWaiterRole())
                    return next({ statusCode: 404, error: true, errormessage: "Unauthorized: user is not a desk or a waiter" });
                (new (table.getModel()))(req.body).save().then(function (data) {
            ...
            app.get("/login", passport.authenticate('basic', { session: false }), function (req, res, next) {
                ...
                var token_signed = jsonwebtoken.sign(tokendata, "AAAAAAAAA", { expiresIn: '1h' });
