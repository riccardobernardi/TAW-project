...
export class StatsChartsComponent implements OnInit {
  ...
  private showError(err) {
    const errmessage = (err.error) ? err.error.errormessage || err.error.message : err;
  ...
  changedRange($event, field) {
    ...
    if ($event.min_date && $event.max_date && $event.min_date <= $event.max_date) { // if dates exist get the reports
      this.report.get_reports({start: $event.min_date, end: $event.max_date}).toPromise().then((reports: Report[]) => {
        ...
        // if the field is total_orders, eleborates the data in a particular way for handling two types, food and beverage
        if (field === 'total_orders') {
          this.statistics[field] = {data : [{data: reports.map((report) => report[field].dish), label: this.labels[field].dish},
              {data: reports.map((report) => report[field].beverage), label: this.labels[field].beverage}],
            dates: reports.map((report) => report.date)};
        } else {
          this.statistics[field] = {data : [{data: reports.map((report) => report[field]), label: this.labels[field]}],
            dates: reports.map((report) => report.date)};
        }
